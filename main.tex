...
\begin{theorem}[\cite{Jost2017} Theorem 3.2.1]
        Assume $\nabla$ is a affine connection on a Riemannian manifold $(M,g)$. For a fixed vector field $X$, we can define the covariant derivative of arbitrary tensor field along $X$ denote by $\nabla_X: \Gamma^\infty(\otimes^{k,l} TM) \to \Gamma^\infty(\otimes^{k,l} TM)$ by the following rules
        \begin{enumerate}
